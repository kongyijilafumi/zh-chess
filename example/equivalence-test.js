...
if (REGEN) {
  ...
  console.log(
    `[regen] snapshot rewritten: ${positions.length} positions, ${Object.keys(fingerprints).length} unique`
  )
  return
}
...
for (const pen of snapshot.positions) {
  ...
  if (fp === snapshot.fingerprints[pen]) {
    ...
    if (stats.mismatch <= 5) {
      failures.push({
        pen,
        expected: snapshot.fingerprints[pen].slice(0, 300),
        actual: fp.slice(0, 300)
      })
    }
  }
}
