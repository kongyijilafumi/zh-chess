 ...
 */
const peiceSideMap = pieceSideMap;
/**
 * 坐标点
 */
class Point {
 ...
 * 坐标 -> 棋盘位表索引，越界返回 -1
 */
const posIdx = (x, y) => x < 0 || x > 8 || y < 0 || y > 9 ? -1 : x + y * 9;
class Piece {
     ...
     */
    filterMovePoints(list, pl, board) {
     ...
     */
    draw(ctx, startX, startY, gridWidth, gridHeight, gridDiffX, gridDiffY, radius, textColor, bgColor, choosePeiceBorderColor, displayName) {
     ...
     * @param radius 棋子半径
     */
    drawMovePoints(ctx, pl, startX, startY, gridWidth, gridHeight, gridDiffX, gridDiffY, radius, moveColor) {
...
const parseStrToPoint = (str, side, pl) => {
    let strRes;
    ...
    if (parse_reg_1.test(str) && (strRes = parse_reg_1.exec(str))) {
        ...
        // 获得 棋子名字
        const pieceName = getSidePieceName(strRes[2], side);
    ...
    if (parse_reg_2.test(str) && (execRes = parse_reg_2.exec(str))) {
        const pieceName = getSidePieceName(execRes[1], side);
...
function parse_PEN_Str(penStr) {
    ...
    for (let y = 0; y < peiceCodeList.length; y++) {
        ...
        for (let j = 0; j < strLen; j++, px--) {
            const str = pieceCodeStr[j];
            const pieceName = parse_PEN_PeiceName(str);
            if (pieceName) {
                const p_side = str.toLocaleLowerCase() === str ? 'BLACK' : 'RED';
...
function gen_PEN_Str(pl, side) {
    let PENList = Array.from({ length: 10 }, () => []);
    pl.forEach(p => {
...
function diffPenStr(oldStr, newStr) {
    ...
    oldList.forEach(item => {
        const findindex = newList.findIndex(p => p.x === item.x && item.y === p.y && item.side === p.side && p.name === item.name);
        // 找到 说明 没有移动
        if (findindex !== -1) {
        ...
        if (mps.length) {
            const hasMp = mps.find(mp => {
                ...
                const findPointIndex = newList.findIndex(_item => _item.x === mp.x &&
                    _item.y === mp.y &&
...
const initBoardPen = 'rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR w';
...
function exportTextBoard(pieces, options) {
    var _a, _b;
    const style = (_a = options === null || options === void 0 ? void 0 : options.style) !== null && _a !== void 0 ? _a : 'chinese';
    const emptyCell = (_b = options === null || options === void 0 ? void 0 : options.emptyCell) !== null && _b !== void 0 ? _b : (style === 'ascii' ? '.' : '·');
    const showCoords = (options === null || options === void 0 ? void 0 : options.showCoords) !== false;
    const formatCell = options === null || options === void 0 ? void 0 : options.formatCell;
    const resolveDisplayName = options === null || options === void 0 ? void 0 : options.resolveDisplayName;
...
function chainFormatCells(formatters) {
    const list = formatters.filter((f) => typeof f === 'function');
    if (!list.length)
        return undefined;
    return (piece, x, y) => {
        for (let i = 0; i < list.length; i++) {
...
class ZhChess {
    constructor(inputCfg) {
        ...
        let cfg = Object.assign({}, gameDefaultCfg);
        if (inputCfg && typeof inputCfg === 'object') {
            cfg = Object.assign(Object.assign({}, gameDefaultCfg), inputCfg);
        }
     ...
     *
     */
    updateAsync(pos, mov, side, refreshCtx, moveCallback) {
    ...
    update(pos, mov, side, post) {
        ...
        const moveCheck = (cp) => {
            if (this.variant && typeof this.variant.beforeMove === 'function') {
                const before = this.variant.beforeMove(pos, mov, side, this);
                if (before === false) {
            ...
            const movedPeiceList = isMove
                ? this.livePieceList.filter(i => !(i.x === posPeice.x && i.y === posPeice.y))
                : this.livePieceList.filter(i => !((i.x === posPeice.x && i.y === posPeice.y) ||
                    (i.x === cp.eat.x && i.y === cp.eat.y)));
            const newMp = chessOfPeiceMap[posPeice.name](Object.assign(Object.assign({}, posPeice), mov));
            ...
            if (this.variant && typeof this.variant.checkWinner === 'function') {
                ...
                const customWinner = this.variant.checkWinner(this);
            ...
            const captured = isMove ? null : movPeice || null;
            const cb = () => {
                const newPeice = chessOfPeiceMap[posPeice.name](Object.assign(Object.assign({}, posPeice), pos));
                if (!isMove) {
    ...
    draw(ctx) {
        ...
        this.livePieceList.forEach(item => {
            const textColor = item.side === 'BLACK' ? blackPeiceTextColor : redPeiceTextColor, bgColor = item.side === 'BLACK' ? blackPeiceBackground : redPeiceBackground;
        ...
        if (this.lastMovePiece) {
            const textColor = this.lastMovePiece.side === 'BLACK' ? blackPeiceTextColor : redPeiceTextColor, bgColor = this.lastMovePiece.side === 'BLACK' ? blackPeiceBackground : redPeiceBackground;
            this.drawOnePiece(ctx, this.lastMovePiece, textColor, bgColor, choosePeiceBorderColor, layout, host);
        }
        if (this.choosePiece) {
            const textColor = this.choosePiece.side === 'BLACK' ? blackPeiceTextColor : redPeiceTextColor, bgColor = this.choosePiece.side === 'BLACK' ? blackPeiceBackground : redPeiceBackground;
            ...
            if (this.drawMovePoint && this.gameState !== 'MOVE') {
                ...
                const customMovePts = this.renderer &&
     ...
     * 绘制单枚棋子（优先走自定义渲染器）
     */
    drawOnePiece(ctx, piece, textColor, bgColor, choosePeiceBorderColor, layout, host) {
        const displayName = this.resolvePieceDisplayName(piece);
        const style = { textColor, bgColor, displayName };
        if (this.renderer &&
    ...
    drawChessLine(ctx) {
        ...
        // 炮位 兵位 坐标的 ∟符号
        for (let i = 0; i < 9; i += 2) {
            const width = gridWidth * 0.15;
            const padding = gridWidth * 0.1;
            for (let j = 0; j < 2; j++) {
                const addx = j === 0 ? -padding : +padding;
                const addy = j === 0 ? +padding : -padding;
                const addw = j === 0 ? +width : -width;
                for (let z = 0; z < 2; z++) {
                    // 红 黑
                    const y = z % 2 === 0 ? startY + 3 * gridHeight : startY + 6 * gridHeight;
                    // 左右两边
                    for (let w = 0; w < 2; w++) {
                        const x = w % 2 === 0 ? startX + i * gridWidth + addx : startX + i * gridWidth - addx;
                        const aw = w % 2 === 0 ? -addw : +addw;
            ...
            if (i - 1 === 1 || i - 1 === 7) {
                for (let j = 0; j < 2; j++) {
                    ...
                    const addy = j === 0 ? +padding : -padding;
                    const addw = j === 0 ? +width : -width;
                    const x1 = startX + (i - 1) * gridWidth;
                    for (let z = 0; z < 2; z++) {
                        const y = (z % 2 === 0 ? startY + 2 * gridHeight : startY + 7 * gridHeight) + addy;
                        for (let w = 0; w < 2; w++) {
                            const x = w % 2 === 0 ? x1 + addx : x1 - addx;
                            const aw = w % 2 === 0 ? -addw : +addw;
                            ctx.beginPath();
                            ctx.moveTo(x, y);
                            ctx.lineTo(x + aw, y);
     ...
     * @returns 移动结果
     */
    moveStrAsync(str, side, refreshCtx) {
        if (!this.checkGameState()) {
     ...
     * @returns 是否安全
     */
    checkGeneralInTrouble(side, pos, cp, pl, board) {
        ...
        const pieceInfo = Object.assign(Object.assign({}, pos), toP);
        ...
        try {
            ...
            const isEnemy = (pc) => pc.side === enemySide;
            // 注意：继承链 RookPiece<-CannonPiece、HorsePiece<-ElephantPiece/SoldierPiece，
            // instanceof 会误判，必须用精确的 constructor 判断
            const isAttackLine = (pc) => (pc.constructor === RookPiece || pc.constructor === GeneralPiece) && pc.side === enemySide;
            const isCannon = (pc) => pc.constructor === CannonPiece && pc.side === enemySide;
     ...
     * @returns 是否危险
     */
    checkGeneralsFaceToFaceInTrouble(board) {
     ...
     * @returns  返回是否有解
     */
    checkEnemySideInTroubleHasSolution(enemySide, pl, board) {
        const b = board || buildBoardIndex(pl);
        return pl
            .filter(i => i.side === enemySide)
            .some(item => {
     ...
     * @returns {boolean}
     */
    checkEnemySideHasMovePoints(enemySide, pl, board) {
        // 当前棋子列表
        ...
        const hasPeice = enemyList.find(p => {
            ...
            return mps.find(mp => {
                // 移动点 是否有棋子
                const checkPoint = b[posIdx(mp.x, mp.y)] ? { eat: mp } : { move: mp };
     ...
     * 获取当前存活的棋子列表
     */
    get currentLivePieceList() {
        return this.livePieceList.map(item => {
            return chessOfPeiceMap[item.name](Object.assign({}, item));
        });
    }
    /**
    ...
    getPiecesOfSide(side) {
        return this.livePieceList
            ...
            .map(item => chessOfPeiceMap[item.name](Object.assign({}, item)));
    }
    /**
     * 获取当前象棋绘制半径
     */
    ...
    isLegalMove(side, from, to) {
        ...
        const cp = hasEat ? { eat: mp } : { move: mp };
        if (this.checkGeneralInTrouble(side, item, cp, pl, b))
            return false;
        if (this.variant && typeof this.variant.filterMoves === 'function') {
            const move = {
     ...
     * @param options 导出选项
     */
    exportTextBoard(options) {
        const pluginFormat = chainFormatCells(this.plugins.map(p => p.formatCell));
        const formatCell = (options === null || options === void 0 ? void 0 : options.formatCell)
            ? chainFormatCells([options.formatCell, pluginFormat])
            : pluginFormat;
        return exportTextBoard(this.livePieceList, Object.assign(Object.assign({}, options), { formatCell, resolveDisplayName: piece => this.resolvePieceDisplayName(piece) }));
    }
    /**
     * 注册扩展插件
     *
    ...
    unuse(name) {
        ...
        let nextRenderer = this.baseRenderer;
        let nextVariant = this.baseVariant;
    ...
    drawLastMovePoint(ctx) {
        if (!this.choosePiece && this.lastMovePoint && ctx) {
            ...
            const gradient = ctx.createRadialGradient(x, y, this.radius * 0.05, x, y, this.radius * 0.8);
