...
game.gameStart('RED')
for (const { pen, check } of POSITIONS.map(p => ({ pen: p, check: false })).concat(
  CHECK_POSITIONS.map(p => ({ pen: p, check: true }))
)) {
  ...
  bucket.checks++
  console.log(
    `PEN ${pen.slice(0, 30)}... side=${side} moves/round=${Math.round(res.totalMove / ROUNDS)}${check ? ' [CHECK]' : ''}`
  )
}
